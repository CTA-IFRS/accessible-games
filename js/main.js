$(function () 
{
  ...
  $(document).on('show.bs.modal', '.modal', function (event) { 
    modalTrigger = event.relatedTarget || document.activeElement; stopScan(); 
  });
  ...
  function renderColmeia() {
    ...
    $fields.append($('<div class="teacher-layout teacher-layout-colmeia">').append($form, $saved));
  }
  ...
  function renderForca() {
    ...
    function saveWords() { 
      localStorage.setItem(FORCA_WORDS_KEY, JSON.stringify(words)); 
    }
    ...
    $fields.append($('<div class="teacher-layout teacher-layout-forca">').append($categoryForm, $('<div class="teacher-main-column">').append($wordForm, $saved)));
  }
  ...
  $dialog.on('click', function (event) { 
    if (event.target === this) closeOptions(); 
  });

  $(document).on('keydown.hubOptions', function (event) { 
    if (event.key === 'Escape' && !$dialog.prop('hidden')) closeOptions(); 
  });
