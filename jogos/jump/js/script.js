$(function () {
    $('#jumpBackToMenu').on('click', function () { window.GameCommon.backToMenu(); });
    ...
    function startScanning(containerSelector) {
        if (containerSelector === '#start-screen') return;
        stopScanning();
    ...
    $(document).on('keydown.jumpScan', (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
            const overlayOpen = !gameOverScreen.hidden || $('#gameOverMenu').is(':visible');
            const menuOpen = !startScreen.hidden;
    ...
    function gameOver() {
        ...
        window.showGameOver(`Pontuação: ${score}. Recorde: ${highScore}.`);
    }
    ...
    $(document).on('click.jumpGame', (event) => {
        const isMenuScreen = !startScreen.hidden || !gameOverScreen.hidden || $('#gameOverMenu').is(':visible');
        
        if (clickLock) return; // evita clique imediato após iniciar jogo

        if (isMenuScreen) {
            if (!gameOverScreen.hidden || $('#gameOverMenu').is(':visible')) return;
    ...
    $('.gameboard').prop('hidden', true);
    window.GameModule = {
        start: function (settings) { setMode(settings.difficulty || 'medium'); $('.gameboard').prop('hidden', false); initGame(); },
        stop: function () { gameRunning = false; if (animationId) cancelAnimationFrame(animationId); },
